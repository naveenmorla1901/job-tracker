...
def get_morganstanley_jobs(roles, days=7):
    """
    Main function to retrieve Morgan Stanley jobs structured by roles
    ...
    def fetch_role_jobs(target_role):
        ...
        base_url = "https://ms.wd5.myworkdayjobs.com/wday/cxs/ms/External/jobs"
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_ms_job, job, cutoff_date): job 
    ...
    def process_ms_job(job, cutoff_date):
        """Process individual job details"""
        try:
            job_url = f"https://ms.wd5.myworkdayjobs.com/en-US/External{job.get('externalPath', '')}"
            metadata = get_ms_job_details(job_url)
            
            if not metadata.get('datePosted'):
                return None
                
            post_date = parse_ms_date(metadata['datePosted'])
            if post_date and post_date >= cutoff_date:
                return format_ms_job_data(job, metadata)
                
        except Exception as e:
            print(f"Error processing job: {str(e)}")
        return None

    def get_ms_job_details(job_url):
            ...
            if script:
                try:
                    ...
                    return {
                        ...
                        'description': clean_ms_description(data.get('description', ''))
                    }
    ...
    def format_ms_job_data(job, metadata):
        ...
        return {
            ...
            "job_url": f"https://ms.wd5.myworkdayjobs.com/en-US/External{job.get('externalPath', '')}",
            "date_posted": format_ms_date(metadata['datePosted']),
            "employment_type": metadata.get('employmentType', 'N/A'),
            "description": metadata.get('description', 'N/A')
        }

    def extract_ms_job_id(job):
