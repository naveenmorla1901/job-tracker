...
def get_kohls_jobs(roles, days=7):
    """Main function to retrieve Kohl's jobs structured by roles"""
    ...
    def fetch_role_jobs(target_role):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_kohls_job, job, cutoff_date): job
...
def process_kohls_job(job, cutoff_date):
    try:
        ...
        post_date = parse_kohls_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_kohls_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_kohls_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_kohls_description(data.get('description', ''))
                }
...
def format_kohls_data(job, metadata):
    return {
...
#     jobs_data = get_kohls_jobs(roles=kohls_roles, days=7)
