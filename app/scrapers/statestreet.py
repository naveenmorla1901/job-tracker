...
def get_statestreet_jobs(roles, days=7):
    """Main function to retrieve State Street jobs structured by roles"""

    def fetch_role_jobs(target_role):
        """Fetch jobs for a single role"""
        base_url = "https://statestreet.wd1.myworkdayjobs.com/wday/cxs/statestreet/Global/jobs"
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_ss_job, job, cutoff_date): job
...
def process_ss_job(job, cutoff_date):
    try:
        job_url = f"https://statestreet.wd1.myworkdayjobs.com/en-US/Global{job.get('externalPath', '')}"
        metadata = get_ss_job_details(job_url)

        if not metadata.get('datePosted'):
            return None

        post_date = parse_ss_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_ss_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_ss_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_ss_description(data.get('description', ''))
                }
...
#     jobs_data = get_statestreet_jobs(roles=roles_to_check, days=7)
