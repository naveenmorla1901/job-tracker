...
def get_autodesk_jobs(roles, days=7):
    """Main function to retrieve Autodesk jobs structured by roles"""
    ...
    def fetch_role_jobs(target_role):
        ...
        headers = {
            ...
            "Referer": "https://autodesk.wd1.myworkdayjobs.com/Ext"
        }
        ...
        try:
            ...
            for job in data.get('jobPostings', []):
                job_id = extract_autodesk_job_id(job)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_autodesk_job, job, cutoff_date): job
    ...
    # Helper functions
    def process_autodesk_job(job, cutoff_date):
        try:
            job_url = f"https://autodesk.wd1.myworkdayjobs.com/en-US/Ext{job.get('externalPath', '')}"
            metadata = get_autodesk_job_details(job_url)

            if not metadata.get('datePosted'):
                return None

            post_date = parse_autodesk_date(metadata['datePosted'])
            if post_date and post_date >= cutoff_date:
                return format_autodesk_job_data(job, metadata)
        except Exception as e:
            print(f"Error processing job: {str(e)}")
        return None

    def get_autodesk_job_details(job_url):
        try:
            ...
            if script:
                try:
                    ...
                    return {
                        ...
                        'description': clean_autodesk_description(data.get('description', ''))
                    }
    ...
    def format_autodesk_job_data(job, metadata):
        return {
            "job_title": job.get('title', 'N/A'),
            "job_id": extract_autodesk_job_id(job),
            "location": job.get('locationsText', 'N/A'),
            "job_url": f"https://autodesk.wd1.myworkdayjobs.com/en-US/Ext{job.get('externalPath', '')}",
            "date_posted": format_autodesk_date(metadata['datePosted']),
            "employment_type": metadata.get('employmentType', 'N/A'),
            "description": metadata.get('description', 'N/A')
        }

    def extract_autodesk_job_id(job):
        try:
    ...
    def parse_autodesk_date(date_str):
    ...
    def format_autodesk_date(date_str):
        try:
            return datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%Y-%m-%d")
        except:
            return date_str

    def clean_autodesk_description(desc):
        if not desc: return 'N/A'
...
#     jobs_data = get_autodesk_jobs(roles=roles_to_check, days=7)
