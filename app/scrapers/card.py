...
def get_card_jobs(roles, days=7):
    """Main function to retrieve Card jobs structured by roles"""

    def fetch_role_jobs(target_role):
        """Fetch jobs for a single role"""
        base_url = "https://card.wd5.myworkdayjobs.com/wday/cxs/card/Search/jobs"
        payload = {
            "appliedFacets": {},
        ...
        headers = {
            ...
            "Referer": "https://card.wd5.myworkdayjobs.com/Search"
        }
        ...
        try:
            ...
            for job in data.get('jobPostings', []):
                job_id = extract_card_job_id(job)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_card_job, job, cutoff_date): job
...
def process_card_job(job, cutoff_date):
    try:
        job_url = f"https://card.wd5.myworkdayjobs.com/Search{job.get('externalPath', '')}"
        metadata = get_card_job_details(job_url)

        if not metadata.get('datePosted'):
            return None

        post_date = parse_card_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_card_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_card_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_card_description(data.get('description', ''))
                }
...
def format_card_job_data(job, metadata):
    return {
        "job_title": job.get('title', 'N/A'),
        "job_id": extract_card_job_id(job),
        "location": job.get('locationsText', 'N/A'),
        "job_url": f"https://card.wd5.myworkdayjobs.com/Search{job.get('externalPath', '')}",
        "date_posted": format_card_date(metadata['datePosted']),
        "employment_type": metadata.get('employmentType', 'N/A'),
        "description": metadata.get('description', 'N/A')
    }

def extract_card_job_id(job):
    try:
...
def clean_card_description(desc):
    if not desc:
        return 'N/A'
...
#     jobs_data = get_card_jobs(roles=roles_to_check, days=7)
