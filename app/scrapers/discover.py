...
def get_discover_jobs(roles, days=7):
    """Main function to retrieve Discover jobs structured by roles"""
    ...
    def fetch_role_jobs(target_role):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_discover_job, job, cutoff_date): job
...
def process_discover_job(job, cutoff_date):
    try:
        ...
        post_date = parse_discover_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_discover_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing Discover job: {str(e)}")
    return None

def get_discover_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_discover_description(data.get('description', ''))
                }
        ...
        print(f"Error fetching Discover details: {str(e)}")
        return {}

def format_discover_job_data(job, metadata):
    return {
...
def format_discover_date(date_str):
    try:
        return datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%Y-%m-%d")
    except:
        return date_str

def clean_discover_description(desc):
    if not desc:
        return 'N/A'
...
#     roles_to_check = ["Data Scientist", "Cybersecurity Analyst"]
#     jobs_data = get_discover_jobs(roles=roles_to_check, days=7)
