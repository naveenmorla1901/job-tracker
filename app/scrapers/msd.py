import requests
from bs4 import BeautifulSoup
import json
from datetime import datetime, timedelta
import concurrent.futures

def get_msd_jobs(roles, days=7):
    """Main function to retrieve MSD jobs structured by roles"""

    def fetch_role_jobs(target_role):
        """Fetch jobs for a single role"""
        base_api_url = "https://msd.wd5.myworkdayjobs.com/wday/cxs/msd/SearchJobs/jobs"

        payload = {
            "appliedFacets": {"locationCountry": ["bc33aa3152ec42d4995f4791a106ed09"]},
            "searchText": target_role,
            "limit": 20,
            "offset": 0
        }

        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
            "Referer": "https://msd.wd5.myworkdayjobs.com/SearchJobs"
        }

        try:
            response = requests.post(base_api_url, json=payload, headers=headers)
            response.raise_for_status()
            data = response.json()

            seen_ids = set()
            cutoff_date = datetime.now() - timedelta(days=days)
            results = []

            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = []
                for job in data.get('jobPostings', []):
                    job_id = extract_msd_job_id(job)
                    if job_id and job_id not in seen_ids:
                        seen_ids.add(job_id)
                        futures.append(executor.submit(
                            process_msd_job,
                            job,
                            cutoff_date
                        ))

                for future in concurrent.futures.as_completed(futures):
                    job_data = future.result()
                    if job_data:
                        results.append(job_data)

            return sorted(results, key=lambda x: x['date_posted'], reverse=True)

        except Exception as e:
            print(f"Error fetching {target_role} jobs: {str(e)}")
            return []

    # Structure results by role
    structured_results = {}
    for role in roles:
        structured_results[role] = fetch_role_jobs(role)

    return structured_results

# Helper functions (keep the same implementations from original code)
def process_msd_job(job, cutoff_date):
    """Process individual job listing"""
    try:
        job_url = f"https://msd.wd5.myworkdayjobs.com/en-US/SearchJobs{job.get('externalPath', '')}"
        metadata = get_msd_job_metadata(job_url)
        if not metadata:
            return None

        post_date = parse_wday_date(metadata.get('datePosted'))
        if not post_date or post_date < cutoff_date:
            return None

        return {
            "job_title": job.get('title', 'N/A'),
            "job_id": extract_msd_job_id(job),
            "location": job.get('locationsText', 'N/A'),
            "job_url": job_url,
            "date_posted": post_date.strftime("%Y-%m-%d"),
            "employment_type": metadata.get('employmentType', 'N/A'),
            "description": clean_description(metadata.get('description', 'N/A'))
        }
    except Exception as e:
        print(f"Error processing job: {str(e)}")
        return None

def get_msd_job_metadata(job_url):
    """Fetch detailed job metadata from individual job page"""
    try:
        response = requests.get(job_url, timeout=10)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')

        script = soup.find('script', {'type': 'application/ld+json'})
        if script:
            try:
                data = json.loads(script.string)
                return {
                    "datePosted": data.get('datePosted'),
                    "employmentType": data.get('employmentType'),
                    "description": data.get('description', '')
                }
            except json.JSONDecodeError:
                pass

        return {
            "datePosted": (soup.find('meta', {'property': 'og:article:published_time'}) or {}).get('content'),
            "employmentType": (soup.find('meta', {'name': 'employmentType'}) or {}).get('content'),
            "description": (soup.find('meta', {'name': 'description'}) or {}).get('content')
        }
    except Exception as e:
        print(f"Error fetching metadata: {str(e)}")
        return {}

def extract_msd_job_id(job):
    """Extract clean job ID from Workday response"""
    try:
        return job['externalPath'].split('_')[-1].split('/')[-1]
    except:
        return job.get('bulletFields', ['N/A'])[0]

def parse_wday_date(date_str):
    """Parse Workday date formats"""
    try:
        return datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%f%z")
    except:
        try:
            return datetime.fromisoformat(date_str.replace('Z', ''))
        except:
            return None

def clean_description(desc):
    """Clean and truncate job descriptions"""
    if not desc or desc == 'N/A':
        return 'N/A'
    cleaned = BeautifulSoup(desc, 'html.parser').get_text(separator=' ')
    return ' '.join(cleaned.split()[:200]) + '...'

# Example usage:
# if __name__ == "__main__":
#     roles_to_check = [
#         "Clinical Research",
#         "Regulatory Affairs",
#         "Medical Advisor",
#         "Pharmacovigilance",
#         "Biostatistician",
#         "Quality Control"
#     ]

#     jobs_data = get_msd_jobs(roles=roles_to_check, days=7)
#     print(json.dumps(jobs_data, indent=2, ensure_ascii=False))