...
def get_lilly_jobs(roles, days=7):
    """Main function to retrieve Eli Lilly jobs structured by roles"""
    ...
    def fetch_role_jobs(target_role):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_lilly_job, job, cutoff_date): job
...
def process_lilly_job(job, cutoff_date):
    try:
        ...
        post_date = parse_lilly_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_lilly_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_lilly_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_lilly_description(data.get('description', ''))
                }
...
def format_lilly_job_data(job, metadata):
    return {
...
#     jobs_data = get_lilly_jobs(roles=roles_to_check, days=7)
