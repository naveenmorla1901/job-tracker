...
def get_deluxe_jobs(roles, days=7):
    """Main function to retrieve Deluxe jobs structured by roles"""

    def fetch_role_jobs(target_role):
        """Fetch jobs for a single role"""
        base_url = "https://deluxe.wd5.myworkdayjobs.com/wday/cxs/deluxe/USA_CAN/jobs"
        ...
        headers = {
            ...
            "Referer": "https://deluxe.wd5.myworkdayjobs.com/USA_CAN"
        }
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_deluxe_job, job, cutoff_date): job
...
def process_deluxe_job(job, cutoff_date):
    try:
        job_url = f"https://deluxe.wd5.myworkdayjobs.com/en-US/USA_CAN{job.get('externalPath', '')}"
        metadata = get_deluxe_job_details(job_url)

        if not metadata.get('datePosted'):
            return None

        post_date = parse_deluxe_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_deluxe_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_deluxe_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_deluxe_description(data.get('description', ''))
                }
...
def format_deluxe_job_data(job, metadata):
    return {
        "job_title": job.get('title', 'N/A'),
        "job_id": extract_job_id(job),
        "location": job.get('locationsText', 'N/A'),
        "job_url": f"https://deluxe.wd5.myworkdayjobs.com/en-US/USA_CAN{job.get('externalPath', '')}",
        "date_posted": format_deluxe_date(metadata['datePosted']),
...
def parse_deluxe_date(date_str):
    try:
...
#     jobs_data = get_deluxe_jobs(roles=roles_to_check, days=7)
