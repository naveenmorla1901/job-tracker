...
def get_montrose_jobs(roles, days=7):
    """Main function to retrieve Montrose jobs structured by roles"""
    ...
    def fetch_role_jobs(target_role):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_montrose_job, job, cutoff_date): job
...
def process_montrose_job(job, cutoff_date):
    try:
        ...
        post_date = parse_montrose_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_montrose_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_montrose_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_montrose_description(data.get('description', ''))
                }
...
def format_montrose_job_data(job, metadata):
    return {
...
def format_montrose_date(date_str):
    try:
        return datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%Y-%m-%d")
    except:
        return date_str

def clean_montrose_description(desc):
    if not desc:
        return 'N/A'
...
#     jobs_data = get_montrose_jobs(roles=montrose_roles, days=7)
