...
def get_deutsche_jobs(roles, days=7):
    """Main function to retrieve Deutsche Bank jobs structured by roles"""

    def fetch_role_jobs(target_role):
        """Fetch jobs for a single role"""
        base_url = "https://db.wd3.myworkdayjobs.com/wday/cxs/db/DBWebsite/jobs"
        payload = {
            "appliedFacets": {
        ...
        try:
            ...
            for job in data.get('jobPostings', []):
                job_id = extract_deutsche_id(job)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                future_to_job = {
                    executor.submit(process_deutsche_job, job, cutoff_date): job
...
# Helper functions
def process_deutsche_job(job, cutoff_date):
    try:
        job_url = f"https://db.wd3.myworkdayjobs.com/en-US/DBWebsite{job.get('externalPath', '')}"
        metadata = get_deutsche_details(job_url)

        if not metadata.get('datePosted'):
            return None

        post_date = parse_deutsche_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_deutsche_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_deutsche_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_deutsche_description(data.get('description', ''))
                }
...
def format_deutsche_data(job, metadata):
    return {
        "job_title": job.get('title', 'N/A'),
        "job_id": extract_deutsche_id(job),
        "location": job.get('locationsText', 'N/A'),
        "job_url": f"https://db.wd3.myworkdayjobs.com/en-US/DBWebsite{job.get('externalPath', '')}",
        "date_posted": format_deutsche_date(metadata['datePosted']),
        "employment_type": metadata.get('employmentType', 'N/A'),
        "description": metadata.get('description', 'N/A')
    }

def extract_deutsche_id(job):
    try:
        return job['externalPath'].split('_')[-1].split('/')[-1]
    except:
        return job.get('bulletFields', ['N/A'])[0]

def parse_deutsche_date(date_str):
    formats = [
        "%Y-%m-%dT%H:%M:%S.%f%z",
        "%Y-%m-%dT%H:%M:%S%z",
        "%Y-%m-%d"
...
def format_deutsche_date(date_str):
    try:
        return datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%Y-%m-%d")
    except:
        return date_str

def clean_deutsche_description(desc):
    if not desc:
        return 'N/A'
...
#     jobs_data = get_deutsche_jobs(roles=deutsche_roles, days=7)
