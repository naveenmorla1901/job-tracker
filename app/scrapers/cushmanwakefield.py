...
def get_cushmanwakefield_jobs(roles, days=7):
    """
    For each role in the roles list, fetch CW jobs posted in the last `days` days.
    Returns a dictionary where keys are role names and values are lists of job dicts.
    """
    aggregated_jobs = {}
    for role in roles:
        aggregated_jobs[role] = fetch_cw_jobs(role, days)
    return aggregated_jobs

def fetch_cw_jobs(target_role, days=7):
    base_url = "https://cw.wd1.myworkdayjobs.com/wday/cxs/cw/external/jobs"
    payload = {
        "appliedFacets": {},
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            future_to_job = {
                executor.submit(process_cw_job, job, cutoff_date): job
...
def process_cw_job(job, cutoff_date):
    try:
        job_url = f"https://cw.wd1.myworkdayjobs.com/en-US/external{job.get('externalPath', '')}"
        metadata = get_cw_job_details(job_url)

        if not metadata.get('datePosted'):
            return None

        post_date = parse_cw_date(metadata['datePosted'])
        if post_date and post_date >= cutoff_date:
            return format_cw_job_data(job, metadata)

    except Exception as e:
        print(f"Error processing job: {str(e)}")
    return None

def get_cw_job_details(job_url):
    try:
        ...
        if script:
            try:
                ...
                return {
                    ...
                    'description': clean_cw_description(data.get('description', ''))
                }
...
def format_cw_job_data(job, metadata):
    return {
        "job_title": job.get('title', 'N/A'),
        "job_id": extract_cw_job_id(job),
        "location": job.get('locationsText', 'N/A'),
        "job_url": f"https://cw.wd1.myworkdayjobs.com/en-US/external{job.get('externalPath', '')}",
        "date_posted": format_cw_date(metadata['datePosted']),
        "employment_type": metadata.get('employmentType', 'N/A'),
        "description": metadata.get('description', 'N/A')
    }

def extract_cw_job_id(job):
    try:
        return job['externalPath'].split('/')[-1]
    except Exception:
        return job.get('bulletFields', ['N/A'])[0]

def parse_cw_date(date_str):
    try:
...
#     cw_roles = ["Software Engineer", "Data Analyst"]
#     jobs_data = get_cushmanwakefield_jobs(roles=cw_roles, days=7)
